...
class RokCommon_Config_Form
{
	...
	public function __construct(&$control, $name, array $options = array())
	{
		...
		// Initialize the RokCommon_Registry data.
	...
	{
		// Make sure there is a valid RokCommon_Config_Form XML document.
		...
		foreach ($data as $k => $v) {
			...
			else if (is_object($v) || RokCommon_Utils_ArrayHelper::isAssociative($v)) {
	...
	/**
	 * Method to get a form field represented as a RokCommon_Config_Field object.
	 *
	 * @param	string	$name	The name of the form field.
	 * @param	string	$group	The optional dot-separated form group path on which to find the field.
	 * @param	mixed	$value	The optional value to use as the default for the field.
	 *
	 * @return	mixed	The RokCommon_Config_Field object for the field or boolean false on error.
	 * @since	1.6
	 */
	...
	public function getFieldAttribute($name, $attribute, $default = null, $group = null)
	{
		...
		if (($element instanceof RokCommon_XMLElement) && ((string) $element[$attribute])) {
	...
	/**
	 * Method to get an array of RokCommon_Config_Field objects in a given fieldset by name.  If no name is
	 * given then all fields are returned.
	 *
	 * @param	string	$set	The optional name of the fieldset.
	 *
	 * @return	array	The array of RokCommon_Config_Field objects in the fieldset.
	 * @since	1.6
	 */
	...
	/**
	 * Method to get an array of RokCommon_Config_Field objects in a given field group by name.
	 *
	 * @param	string	$group	The dot-separated form group path for which to get the form fields.
	 * @param	boolean	$nested	True to also include fields in nested groups that are inside of the
	 * 							group for which to find fields.
	 *
	 * @return	array	The array of RokCommon_Config_Field objects in the field group.
	 * @since	1.6
	 */
	...
	public function load($data, $replace = true, $xpath = false)
	{
		...
		if ((!$data instanceof RokCommon_XMLElement) && (!is_string($data))) {
			return false;
		}

		// Attempt to load the XML if a string.
		if (is_string($data)) {
			$data = RokCommon_Config_Form::getXML($data, false);
	...
	public function setField(& $element, $group = null, $replace = true)
	{
		...
		if ($replace && !empty($old) && ($old instanceof RokCommon_XMLElement)) {
		...
		if ($group) {
			...
			if (isset($fields[0]) && ($fields[0] instanceof RokCommon_XMLElement)) {
	 ...
	 * @param	boolean	$replace	True to replace existinRokCommon_Utils_XMLElementdy exist.
	 *
	 * @return	boolean	True on success.
	 * @since	1.6
	 */
	 ...
	 *
	 * Validation warnings will be pushed into RokCommon_Config_Form::errors and should be
	 * retrieved with RokCommon_Config_Form::getErrors() when validate returns boolean false.
	 *
	...
	/**
	 * Method to load, setup and return a RokCommon_Config_Field object based on field data.
	 *
	 * @param	string	$element	The XML element object representation of the form field.
	 * @param	string	$group		The optional dot-separated form group path on which to find the field.
	 * @param	mixed	$value		The optional value to use as the default for the field.
	...
	protected function loadField($element, $group = null, $value = null)
	{
		...
		// Load the RokCommon_Config_Field object for the field.
		...
		// Setup the RokCommon_Config_Field object.
		$field->setForm($this);
	 ...
	 *
	 * @return	mixed	RokCommon_Config_Field object on success, false otherwise.
	 * @since	1.6
	 */
	protected function loadFieldType($type, $new = true)
	{
		return RokCommon_Config_FormHelper::loadFieldType($type, $new);
	}


	/**
	 * Method to synchronize any field, form or group paths contained in the XML document.
	 *
	 * @return	boolean	True on success.
	 * @since	1.6
	 */
	protected function syncPaths()
	{
        ...
        // Get any addgrouppath attributes from the form definition.
		$paths = $this->xml->xpath('//*[@addgrouppath]/@addgrouppath');
		$paths = array_map('strval', $paths ? $paths : array());
	...
	/**
	 * Proxy for {@link RokCommon_Config_FormHelper::addFieldPath()}.
	 *
	 ...
	 * @since	1.6
	 */
	public static function addFieldPath($new)
	{
        if (!empty($new)){
            RokCommon_Config_FieldLoader::addPath($new);
        }
		return RokCommon_Config_FormHelper::getPaths();
	}

	/**
	 * Proxy for {@link RokCommon_Config_FormHelper::addFormPath()}.
	 *
	 * @param	mixed	$new	A path or array of paths to add.
	 *
	 * @return	array	The list of paths that have been added.
	...
	{
		return RokCommon_ConfigForm_Helper::addFormPath($new);
	}
	 ...
	 * @param	string	$xpath		An optional xpath to search for the fields.
	 *
	 * @return	object	RokCommon_Config_Form instance.
	...
	public static function getInstance(&$control, $name, $data = null, $options = array(), $replace = true, $xpath = false)
	{
		...
		if (!isset($forms[$name])) {
			...
			$forms[$name]= new RokCommon_Config_Form($control, $name, $options);
    ...
    public static function getXML($data, $isFile = true)
	{
		...
		if (empty($xml)) {
            //TODO handle errors
			// There was an error
//			JError::raiseWarning(100, rc__('JLIB_UTIL_ERROR_XML_LOAD'));
//
//			if ($isFile) {
...
//			}
	...
	protected function & findFullFieldsByFieldset($name, $xml = null)
	{
		...
		// Make sure there is a valid RokCommon_Config_Form XML document.
		if (!$xml instanceof RokCommon_XMLElement) {
			return $false;
		}
    ...
    public function  & getSubFields(&$xml) {
        // Initialize variables.
        $fields = array();
	...
	{
		return RokCommon_Config_FormHelper::loadGroupType($type, $new);
	}
	 ...
	 * @since	1.6
	 */
	public static function addGroupPath($new)
	{
         if (!empty($new)){
            RokCommon_Config_GroupLoader::addPath($new);
        }
		return RokCommon_Config_GroupLoader::getPaths();
	}
