...
class RTConfigFieldGroupedList extends RokCommon_Config_Field
{
		...
		foreach ($this->element->children() as $element) {
			...
			switch ($element->getName())
			{
				...
				case 'option':
					...
					$tmp = RokCommon_Config_HTML_Select::option(
						($element['value']) ? (string) $element['value'] : trim((string) $element),
						rc__(trim((string) $element)), 'value', 'text',
				...
				case 'group':
					...
					foreach ($element->children() as $option)
					{
						...
						// Create a new option object based on the <option /> element.
						$tmp = RokCommon_Config_HTML_Select::option(
							($element['value']) ? (string) $element['value'] : rc__(trim((string) $element)),
							rc__(trim((string) $element)), 'value', 'text',
							((string) $element['disabled']=='true'));
