...
dom.insertAfter(row,targetRow);});dom.removeClass(dom.select('td.mceSelected,th.mceSelected'),'mceSelected');};function getPos(target){var pos;each(grid,function(row,y){each(row,function(cell,x){if(cell.elm==target){pos={x:x,y:y};return false;}});return!pos;});return pos;};function setStartCell(cell){startPos=getPos(cell);};function findEndPos(){var pos,maxX,maxY;maxX=maxY=0;each(grid,function(row,y){each(row,function(cell,x){var colSpan,rowSpan;if(isCellSelected(cell)){cell=grid[y][x];if(x>maxX)
maxX=x;if(y>maxY)
...
if((value=dom.getAttrib(node,'height'))){dom.setStyle(node,'height',value);dom.setAttrib(node,'height','');}}});ed.onGetContent.add(function(ed,o){if(!tinymce.isIE){o.content=o.content.replace(/<td([^>]*)>&nbsp;<\/td>/gi,'<td$1></td>');}});ed.onPreInit.add(function(ed,o){if(ed.settings.schema=='html5'&&ed.settings.validate){ed.parser.addNodeFilter('table',function(nodes){for(var i=0,len=nodes.length;i<len;i++){var node=nodes[i];if(node.attr('border')){node.attr('border','1');}else{node.attr('border',null);}}});}
