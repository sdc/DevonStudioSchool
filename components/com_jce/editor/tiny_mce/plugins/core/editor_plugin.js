...
return t.hideDialog();DOM.show(t.id+'_dialog');this.storeSelection();if(s.url){var iframe=DOM.get(t.id+'_iframe');iframe.src=s.url;}
...
t.hideDialog();});}
t.isDialogVisible=1;},storeSelection:function(){this.editor.bookmark=this.editor.selection.getBookmark(1);},restoreSelection:function(){this.editor.selection.moveToBookmark(this.editor.bookmark);},renderDialog:function(){var t=this,m,s=this.settings,w,v,ed=this.editor;s['class']+=' '+ed.getParam('skin')+'Skin';if(v=ed.getParam('skin_variant')){s['class']+=' '+ed.getParam('skin')+'Skin'+v.substring(0,1).toUpperCase()+v.substring(1);}
...
t.isDialogVisible=0;t.onHideDialog.dispatch(t);},postRender:function(){var t=this,s=t.settings;Event.add(t.id,'click',function(){if(!t.isDisabled()){if(s.onclick)
