 ...
 */
(function($){$.support.canvas=!!document.createElement('canvas').getContext;$.widget("ui.tips",{options:{speed:150,position:'top center',opacity:0.9,className:'tooltip',offsets:{'x':16,'y':18},width:200,fixed:true,sticky:false},_init:function(options){var self=this;$.extend(this.options,options);if(this.options.sticky){self._pin();}else{if($(this.element).hasClass('tooltip-cancel-ondrag')){this._cancelOnDrag();}
$(this.element).click(function(e){if(this.nodeName=='A'||$('a',this).length||$(this).hasClass('tooltip-cancel-ondrag')){return;}
if($('#jce-tooltip').hasClass('sticky')){return self._unpin();}else{return self._pin();}});}
$(this.element).hover(function(e){if($('#jce-tooltip').hasClass('sticky')||$(this).hasClass('nohover')){return;}
return self._start(e);},function(){if($('#jce-tooltip').hasClass('sticky')||$(this).hasClass('nohover')){return;}
...
this._unpin();},_cancelOnDrag:function(){var self=this;$(this.element).bind('mousedown',function(){$(this).addClass('nohover');self._end();$(this).data('title',$(this).attr('title')).attr('title','');}).bind('mouseup',function(){$(this).removeClass('nohover');$(this).attr('title',$(this).data('title'));});},_pin:function(){$('#jce-tooltip').addClass('sticky');$('span.ui-icon-close','#jce-tooltip').show();},_unpin:function(){$('#jce-tooltip').removeClass('sticky');$('span.ui-icon-close','#jce-tooltip').hide();},_position:function(){var $tips=$('#jce-tooltip');var p=$(this.element).offset();var o=this.options.offsets;var tip={'x':$tips.outerWidth(),'y':$tips.outerHeight()};var pos={x:p.left-tip.x/2+$(this.element).outerWidth()/2,y:p.top-(tip.y+o.y)};var position=this.options.position;var scrollTop=$(document).scrollTop();if(pos.y<0||pos.y<scrollTop){$tips.removeClass('jce-'+this.options.className+'-top');position=position.replace('top','bottom');$tips.addClass('jce-'+this.options.className+'-bottom');pos.y=p.top+o.y+10;}else{$tips.removeClass('jce-'+this.options.className+'-bottom');position=position.replace('bottom','top');$tips.addClass('jce-'+this.options.className+'-top');}
...
$tips.css({top:pos.y,left:pos.x});},destroy:function(){$.Widget.prototype.destroy.apply(this,arguments);}});$.extend($.ui.tips,{version:"2.2.8.1"});})(jQuery);