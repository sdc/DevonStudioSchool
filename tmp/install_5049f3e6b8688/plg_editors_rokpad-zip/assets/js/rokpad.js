...
e.cancel().post(c.toQueryString().parseQueryString());},saveRequest:function(){document.getElements("[data-rokpad-save] i").addClass("spinner");},saveSuccess:function(b){var e=new Element("div").set("html",b),d=document.getElement("form[name=adminForm"),c=e.getElements("form[name=adminForm] input[type=hidden]");
if(e.getElements("#toolbar-apply, .icon-32-apply").length&&c.length){d.getElements("input[type=hidden]").dispose();c.inject(d);}document.getElements("[data-rokpad-savedate]").set("text",(new Date()).format("%d %b, %T"));
...
this.changeOptions(f,b);}},this);},_extrasFixes:function(e,c){var b=c.getParent(".repeatable-list .repeatable-element.hidden"),d=c.getElement("! > .repeatable-list ~ p.add a");
if(b&&d){if(!d.retrieve("rokpad:zoo:add",false)){d.store("rokpad:zoo:add",true);d.addEvent("click",function(){var h=c.getParent(".repeatable-list").getElements(".repeatable-element:not(.hidden)").getLast(),g=h.getElement("[data-rokpad-editor]"),f=g.get("data-rokpad-editor");
this.editors[f].getEditor().resize();this.editors[f].getEditor().focus();}.bind(this));}}},_rearrangeHeight:function(g,e){var f=this.editors[g].wrapper,c=f.getElement("[data-rokpad-container]"),d=f.getElement("[data-rokpad-actionbar]"),b=d.getSize().y-Math.abs(e||0);
