...
}if(!this.textarea){throw new Error('Original textarea "'+b.textarea+'" not found in the DOM.');}this.textarea.inject(this.textarea.getParent("form"));
this.editor=ace.edit(this.container);this.insert(this.textarea.get("value"));var d=this.editor.getSession().getUndoManager();d.reset.delay(1,d);this.attachEvents();
this.container.getElement(".ace_layer.ace_text-layer").set("contenteditable","");return this;},getEditor:function(){return this.editor;},attachEvents:function(){var b=this.options;
this.textarea.addEvent("blur",function(){this.setValue(this.textarea.get("value"));}.bind(this));this.editor.on("blur",function(){this.textarea.set("value",this.getValue());
}.bind(this));this.editor.on("change",function(c){this.fireEvent("change",c,1);}.bind(this));document.id(this.editor.renderer.scroller).addEvent("scroll",function(){var d=this.editor.renderer,e=d.$gutter,c=d.scroller,f=c.scrollLeft;
document.id(e)[f<5?"removeClass":"addClass"]("horscroll");}.bind(this));this.editor.commands.addCommand({name:"find",bindKey:{win:"Ctrl-F",mac:"Command-F"},exec:function(d){var c=this.wrapper.getElement("[data-rokpad-action-method=find] input");
